    ...
    yargs => {
      ...
      yargs.options({
        ...
        'resume': {
          ...
          demandOption: false,
          default: false,
          describe: 'Resumes the command from a "streams-status.json" file assuming an error occurred (i.e. rate-limiting).',
          type: 'boolean'
        }
      })
    },
    argv => {
      console.info(` Running for org: ${argv.org}, project: ${argv.project} with dry-run set to: ${argv.dryRun}, days since seen data set to: ${argv.days}, resume from file set to: ${argv.resume}`);
...
function batchDeleteStreams(argv, ids, statuses) {
  let batches = split(ids);
  return Promise.map(batches, (b, i) => {
...
function resume(argv, youngest, oldest) {
  return getStreamStatus() // load file (get list of streams with current status (unknown, inactive or deleted))
    .then((streamStatuses) => { // check any statuses of streams that are marked unknown
      let streamIdsToBeChecked = Object.keys(streamStatuses).filter((id) => {
        return streamStatuses[id] === 'unknown';
      });
      return batchCheckStreamStatuses(argv, youngest, oldest, streamIdsToBeChecked, streamStatuses);
    })
    .then((statuses) => storeStreamStatus(statuses)) // save our progress to file (regardless of whether it error'd out or not on the previous step for some batches)
    .then((statuses) => deleteStreams(argv, statuses)) // try and delete the streams that were marked inactive
    .then((statuses) => storeStreamStatus(statuses)) // save our progress to file again (regardless of whether it error'd out or not on the previous step for some batches)
...
function storeStreamStatus(status) {
  return fs.writeFile('./streams-status.json', JSON.stringify(status)).then(() => { return status; });
}
...
function filterBatch(args, youngest, oldest, batch, statuses) {
  ...
  return Promise.all(reqs)
    ...
    .catch((err) => {
      ...
      console.error(`\n Error occurred while checking stream activity for batch, error: ${JSON.stringify(formatError(err))}. Setting status to unknown on this batch.`);
      batch.forEach(streamId => {
...
function deleteBatch(args, batch, statuses) {
  let deletions = [];
  batch.forEach((streamId) => {
    deletions.push(deleteStreamRequest(args, streamId).then(() => {
  ...
  return Promise.all(deletions)
    .catch((err) => {
      ...
      console.error(`\n Error occurred while deleting streams for batch, error: ${JSON.stringify(formatError(err))}. Some streams may not be deleted.`);
      return Promise.resolve();
    });
}
...
function deleteStreamRequest(args, streamId) {
  ...
  return request.delete(`https://api.lightstep.com/public/v0.1/${args.org}/projects/${args.project}/searches/${streamId}`, opts);
}
...
function listStreamsRequest(args, youngest, oldest) {
  ...
  return request.get(`https://api.lightstep.com/public/v0.1/${args.org}/projects/${args.project}/searches`, opts)
    .then((res) => {
      return res.data
...
function timeseriesRequest(args, streamId, youngest, oldest) {
  ...
  return request.get(`https://api.lightstep.com/public/v0.1/${args.org}/projects/${args.project}/searches/${streamId}/timeseries?include-ops-counts=1&include-error-counts=0&youngest-time=${youngest.toISOString()}&oldest-time=${oldest.toISOString()}&resolution-ms=36000000`, opts)
    .then((res) => {
      let ret = {
